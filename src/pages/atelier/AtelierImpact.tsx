            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
              {stats.map((stat, index) => {
                ...
                return (
                  <div
                    key={stat.title}
                    className={`group relative px-4 py-6 text-center transition-all duration-500 hover:-translate-y-1 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/5 hover:border-[#C97A53]/30 ${index % 2 === 0 ? 'sm:pr-4' : 'sm:pl-4'}`}
                  >
                    <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/5 transition-all duration-500 group-hover:border-[#C97A53]/40 group-hover:bg-[#C97A53]">
                      <Icon size={20} strokeWidth={1.4} className="text-white/70 transition-colors duration-500 group-hover:text-white" />
                    </div>

                    <div className="font-serif text-3xl font-light tracking-tight text-white sm:text-4xl">
                      {stat.value}
                      {stat.suffix && <span className="text-[#C97A53]">{stat.suffix}</span>}
                    </div>
